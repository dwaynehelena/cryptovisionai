...
class Dashboard:
    ...
    def _setup_layout(self):
        """Configure dashboard layout with routing"""
            ...
            dbc.Container([
                ...
                dbc.Collapse(
                    dbc.Nav([
                        ...
                        dbc.NavItem(dbc.NavLink("Trading", href="/trading", active="exact")),
        ...
        self.app.layout = html.Div([
            ...
            # Hidden storage - Moved here to be available across all pages
    ...
    def _render_trading_page(self):
        ...
        symbol_options = [{"label": s, "value": s} for s in active_symbols]
        
        return dbc.Container([
            html.H2("Manual Trading", className="mt-4 mb-4"),
            
            dbc.Row([
                # Order Form
                dbc.Col([
                    dbc.Card([
                        dbc.CardHeader("Place Order"),
                        dbc.CardBody([
                            dbc.Form([
                                dbc.Row([
                                    dbc.Col([
                                        ...
                                        dcc.Dropdown(
                                            ...
                                            value=active_symbols[0] if active_symbols else None,
                                            className="mb-3"
                                        )
                                    ])
                                ]),
                                ...
                                dbc.Row([
                                    dbc.Col([
                                        dbc.Label("Amount"),
                                        dbc.Input(id="trade-amount", type="number", step="0.0001", className="mb-3")
                                    ])
                                ]),
                                dbc.Button("Execute Trade", id="execute-trade-btn", color="primary", block=True),
                ...
                dbc.Col([
                    dbc.Card([
                        dbc.CardHeader("Market Information"),
                        dbc.CardBody([
                            html.Div(id="trading-market-info")
                        ])
                    ])
                ], width=8)
            ]),
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            html.Div(id="trading-open-positions")
                        ])
                    ])
                ], className="mb-4")
            ]),
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            html.Div(id="trading-recent-trades")
                        ])
                    ])
                ])
            ]),
            ...
            # Interval for updates
            dcc.Interval(id="trading-update-interval", interval=5000, n_intervals=0)
        ], fluid=True)
    ...
    def _render_models_page(self):
        ...
        # Get model config
        model_config = self.trading_system.config.get("model_config", {})
        feature_set = self.trading_system.config.get("feature_set", "standard")
        
        return dbc.Container([
            html.H2("Model Management", className="mt-4 mb-4"),
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            html.H4("Ensemble Model", className="card-title"),
                            html.H5(
                                "Loaded" if model_loaded else "Not Loaded", 
                                className="text-success" if model_loaded else "text-danger"
                            ),
                            html.Hr(),
                            html.P([html.Strong("Type: "), str(model_config.get("ensemble_type", "Unknown"))]),
                            html.P([html.Strong("Feature Set: "), feature_set]),
                            html.P([html.Strong("Last Update: "), system_status.get("last_update", "Never")])
                        ])
                    ], className="mb-4")
                ], width=6),
                ...
                dbc.Col([
                    dbc.Card([
                        dbc.CardHeader("Model Components"),
                        dbc.CardBody([
                            dbc.ListGroup([
                                dbc.ListGroupItem([
                                    html.Div([
                                        html.H5("LSTM", className="mb-1"),
                                        html.P("Deep Learning for Sequence Data", className="mb-1")
                                    ], className="d-flex w-100 justify-content-between"),
                                ]),
                                dbc.ListGroupItem([
                                    html.Div([
                                        html.H5("XGBoost", className="mb-1"),
                                        html.P("Gradient Boosting Decision Trees", className="mb-1")
                                    ], className="d-flex w-100 justify-content-between"),
                                ]),
                                dbc.ListGroupItem([
                                    html.Div([
                                        html.H5("Transformer", className="mb-1"),
                                        html.P("Attention-based Sequence Model", className="mb-1")
                                    ], className="d-flex w-100 justify-content-between"),
                                ])
                            ])
                        ])
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardHeader("Training Performance (Last Run)"),
                        dbc.CardBody([
                            html.P("Training metrics will appear here after the next training cycle.", className="text-muted")
                        ])
                    ])
                ])
            ])
    ...
    def _render_settings_page(self):
        ...
        # Get current config
        config = self.trading_system.config if self.trading_system.config else {}
        risk_config = config.get("risk_management") or {}
        api_key = config.get("api_key", "")
        masked_key = f"{api_key[:4]}...{api_key[-4:]}" if api_key and len(api_key) > 8 else "Not Configured"
        
        return dbc.Container([
            html.H2("Settings", className="mt-4 mb-4"),
                ...
                dbc.Col([
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            dbc.Form([
                                html.Div([
                                    dbc.Label("Max Position Size (% of Portfolio)"),
                                    dbc.Input(
                                        id="setting-max-pos-size", 
                                        type="number", 
                                        value=risk_config.get("max_position_size", 20.0),
                                        step=1
                                    )
                                ], className="mb-3"),
                                html.Div([
                                    dbc.Label("Stop Loss (%)"),
                                    dbc.Input(
                                        id="setting-stop-loss", 
                                        type="number", 
                                        value=risk_config.get("stop_loss_percent", 3.0),
                                        step=0.1
                                    )
                                ], className="mb-3"),
                                html.Div([
                                    ...
                                    dbc.Input(
                                        id="setting-take-profit", 
                                        type="number", 
                                        value=risk_config.get("take_profit_percent", 6.0),
                                        step=0.1
                                    )
                                ], className="mb-3"),
                                dbc.Button("Save Risk Settings", id="save-risk-settings-btn", color="primary")
                            ]),
                            html.Div(id="risk-settings-status", className="mt-3")
                        ])
                    ], className="mb-4")
                ], width=6),
                    ...
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            html.Div([
                                ...
                                dbc.Input(value=masked_key, disabled=True),
                                dbc.FormText("API keys are configured via .env file for security")
                            ], className="mb-3"),
                            html.Div([
                                ...
                                dbc.RadioItems(
                                    ...
                                    value=self.trading_system.config.get("use_testnet", True),
                                    id="setting-use-testnet",
                                    inline=True
                                )
                            ], className="mb-3")
                        ])
    ...
    def _setup_callbacks(self):
        ...
        def display_page(pathname):
            if pathname == "/trading":
                return self._render_trading_page()
            elif pathname == "/models":
                return self._render_models_page()
        ...
        def execute_trade(n_clicks, symbol, action, amount):
            if not n_clicks:
                return ""
            ...
            try:
                result = self.trading_system.execute_trade(symbol, action, float(amount))
                
                if "error" in result:
                    return dbc.Alert(f"Error: {result['error']}", color="danger")
                
                return dbc.Alert(f"Trade executed successfully: {action.upper()} {amount} {symbol}", color="success")
            except Exception as e:
                return dbc.Alert(f"System Error: {str(e)}", color="danger")

        @self.app.callback(
            [Output("trading-open-positions", "children"),
             Output("trading-recent-trades", "children"),
             Output("trading-market-info", "children")],
            [Input("trading-update-interval", "n_intervals"),
             Input("trade-symbol", "value")]
        )
        def update_trading_page(n, selected_symbol):
            # Get positions
            all_positions = self.trading_system.portfolio.get_all_positions()
            open_positions = all_positions.get("open_positions", [])
            closed_positions = all_positions.get("closed_positions", [])
            
            # Format Open Positions Table
            if open_positions:
                open_df = pd.DataFrame(open_positions)
                open_table = dbc.Table.from_dataframe(
                    open_df[["symbol", "position_type", "entry_price", "amount", "pnl", "pnl_percent"]],
                    striped=True, bordered=True, hover=True
                )
            else:
                open_table = html.P("No open positions")
                
            # Format Recent Trades Table
            if closed_positions:
                closed_df = pd.DataFrame(closed_positions)
                recent_table = dbc.Table.from_dataframe(
                    closed_df[["symbol", "position_type", "entry_price", "exit_price", "pnl", "pnl_percent"]].tail(10),
                    striped=True, bordered=True, hover=True
                )
            else:
                recent_table = html.P("No recent trades")
                
            # Market Info
            market_info = html.Div()
            if selected_symbol:
                try:
                    ticker = self.trading_system.binance_connector.get_ticker(selected_symbol)
                    market_info = html.Div([
                        html.H4(f"{selected_symbol}: {float(ticker['price']):.2f}"),
                        html.P(f"24h Change: {float(ticker['priceChangePercent']):.2f}%", 
                               className="text-success" if float(ticker['priceChangePercent']) > 0 else "text-danger")
                    ])
                except:
                    market_info = html.P("Market data unavailable")
            
            return open_table, recent_table, market_info
        
        # Settings Page Callbacks
        @self.app.callback(
            Output("risk-settings-status", "children"),
            [Input("save-risk-settings-btn", "n_clicks")],
            [State("setting-max-pos-size", "value"),
             State("setting-stop-loss", "value"),
             State("setting-take-profit", "value")]
        )
        def save_risk_settings(n_clicks, max_pos, stop_loss, take_profit):
            if not n_clicks:
                return ""
            
            try:
                # Update config in memory
                if "risk_management" not in self.trading_system.config:
                    self.trading_system.config["risk_management"] = {}
                    
                self.trading_system.config["risk_management"]["max_position_size"] = float(max_pos)
                self.trading_system.config["risk_management"]["stop_loss_percent"] = float(stop_loss)
                self.trading_system.config["risk_management"]["take_profit_percent"] = float(take_profit)
                
                # Update risk manager if initialized
                if self.trading_system.risk_manager:
                    self.trading_system.risk_manager.max_position_size = float(max_pos)
                    self.trading_system.risk_manager.stop_loss_percent = float(stop_loss)
                    self.trading_system.risk_manager.take_profit_percent = float(take_profit)
