...
export default function OrderForm({ symbol, currentPrice }: Props) {
    ...
    const handleOrderTypeChange = (event: React.MouseEvent<HTMLElement>, newType: 'MARKET' | 'LIMIT' | 'STOP_LOSS' | null) => {
        if (newType !== null) {
            setOrderType(newType);
        }
    };

    const handleSideChange = (event: React.MouseEvent<HTMLElement>, newSide: 'BUY' | 'SELL' | null) => {
        if (newSide !== null) {
            setSide(newSide);
        }
    };
